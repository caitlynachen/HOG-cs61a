...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    pldiff = abs((player_score // 10 % 10) - (player_score % 10))
    opdiff = abs((opponent_score // 10 % 10) - (opponent_score % 10))
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        ...
        if (s - pscore) > phigh:
            phigh = s - pscore
            print((s - pscore), "point(s)! That's the biggest gain yet for Player", who)
            phigh = s - pscore
        pscore = s
            
        return announce_highest(who, phigh, pscore)
    return say
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    large = 0
    max = 0
    avgdice = make_averaged(roll_dice, num_samples)
    i = 10
    while i > 0:
        avg = avgdice(i, dice)
        if avg > large:
